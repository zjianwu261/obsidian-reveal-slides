...
**正文块一律用 class="abstract"。** 别的名字（\`body\`、\`content\` 之类）课程 CSS 里没有，
写了等于没有版式。
...
**先想清楚这张图要让人看懂哪一件事**，再决定用什么画。一张图只讲一件事，
方块加箭头总共不超过 7 个 —— 画满了不叫有深度，叫看不清。

四种 type 套得上就用 type（位置和字号由渲染器算，不会出错）。
套不上（要画一个比喻、一个具体场景）才手绘 \`\`\`svg，而且按下面这份规矩来，
**别自由发挥**：SVG 里没有自动排版，坐标算错就是字叠在一起。

  画布   占满整行 viewBox="0 0 900 300"；并排 viewBox="0 0 600 500"。
         四周留 24 的白边，所有东西画在里面。
  文字   字号 ≥ 24（按 900 宽的画布算）。<text> 一律带
         text-anchor="middle" dominant-baseline="middle"，坐标写方块的中心，
         不要自己估基线。一处不超过四五个字。
  方块   rx="8" fill="none" stroke="currentColor" stroke-width="2"。
         **宽度按字数算**：中文一字≈字号，英文数字≈0.6×字号，左右各再留 16。
         算出来比你想画的窄，就把方块加宽，别让字顶出去。
  箭头   在 <defs> 里定义一个 marker，线上写 marker-end="url(#…)"。
         箭头只走水平或垂直，斜线一多就乱。
  颜色   只用三档：currentColor（主）、opacity="0.55"（次要）、
         #c0392b（强调，一张图里最多标两处）。别写死黑色白色，
         深色主题下会看不见。
  排布   同一排的方块 y 相同、高相同、间距相同：先把 x 排成等差数列再落笔。

写完检查三件事：字有没有顶出方块、方块之间有没有重叠、有没有东西越过白边。
