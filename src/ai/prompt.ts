...
**四种 type 都套不上就直接写 \`\`\`svg 手绘一张**，别把想法硬塞进模板 ——
要画一个比喻、一个具体的场景时，模板反而碍事。写法：

  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 300">…</svg>

viewBox 定好宽高比（占满整行用 900×300 这样宽扁的，并排用 600×500 这样偏方的）；
颜色用 currentColor 或课程 CSS 里的配色变量，别写死；字号按 viewBox 900 宽算别小于 22，
投影上才看得清。手绘时字更要少 —— 靠位置、大小、箭头说话，一处别超过四五个字。
