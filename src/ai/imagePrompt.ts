...
就是他真正想让学生带走的东西，挑它。
挑完再问自己一句：这一段里学生**最容易想错的是哪一步**？图要打的就是那一处。
学生本来就懂的东西，画得再漂亮也是白画。
...
**画的永远是一次变化，不是一样东西的样子。** 摆在那儿的静物讲不出道理 ——
得有人动手，动完之后有东西不一样了。
...
## 写成这个格式（五行，一个字段一行）
  ...
  依据：从讲稿里**原样抄一句**（最多两句），这张图画的就是它。
  场景：一句话，这张图借的是什么日常场景。
  构图：整张图怎么摆。只有两种 ——「从左到右三步」或者「左右两格对照」。
  画面：三到五件事，分号隔开，每件写「在哪儿 + 是什么 + 在做什么」。
  重点：整张图最该一眼看到的那一处。

**「依据」必须是讲稿里真有的原话**，不许自己概括、不许改写。
它是给老师一眼核对用的：挑错了句子，他当场就看得出来，
不用等一分钟画完才发现跑偏。抄不出这样一句，说明你还没找到要画的那件事。

**「构图」那两种各有各的用法。**
「从左到右三步」画的是**同一样东西的前后**：左边动手之前、中间那一下动作、
右边变完的样子 —— 不是三样不相干的东西排成一排。
「左右两格对照」两格必须是同一件事的正反面（先改再用 vs 先用再改），
不能是两个话题各占一格；只讲顺序的就写成一条动作线，别分格。

分行是为了好改：不满意换个比喻就重写「场景」那一行，
嫌乱就删「画面」里的一件，别的不动。
...
- **动作链最多三步，别来回倒腾。** 「倒进碗里、再倒回罐子」这种两步搬运，
  画出来只剩乱。能一步到的就不要绕。
- **原理里没动的东西，画面里一下都不许碰。**
  「a = a + b」里 b 只是被读了一眼，算完还是原来那个数 ——
  那画面里对应 b 的那样东西**全程留在原位**：不许被倒出来、拿起来、
  摊开清点、挪个地方。「先拿走再放回去」也不行，那画出来就是被搬空了。
  它只能被「看一眼」「照着数一遍」。
  写完回头数一遍：讲稿里哪几样东西变了、哪几样没变，跟画面对得上才算过。
...
- 除「依据」那一行外，加起来 180 字以内。不要引号、不要再加别的字段。
...
写完核最后一遍：**照着这段画出来，看图的人能看出容易错的是哪一步吗？**
答不上来，说明画的还是那个知识点本身，回去重挑。

**输出**：只输出这五行，不要解释、不要引号、不要代码围栏。`;
...
Scene 那一段是五行：依据、场景、构图、画面、重点。翻的时候把它们捏成一段连贯的英文：

- **「依据」那一行不要翻。** 它是讲稿原话，给老师核对用的，不是画面里的东西 ——
  照着翻，画出来会是一堆中文标语。跳过它。
- **场景**是这幅画的设定，先说它。
