...
| Export Slides as PDF | — | 打开打印视图（浏览器中 打印 → 另存为 PDF） |
| Export Slides as HTML | — | 导出单文件离线 HTML 到导出目录 |
...
| Export | 导出目录 |
| Preview | 面板位置（**默认与笔记并排**，可选独立窗口 / 右侧边栏）、滚动视图阈值、自动刷新、`<grid>`/`<split>` 属性自动补全、版面辅助线 |
