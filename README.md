...
| 级别 | 放哪 | 放什么 | 例子 |
|------|------|--------|------|
| 课程级 | vault 里的 `.css` 文件 | 配色变量 + 全套版式 class | `themes/course.css` |
| 本篇级 | 笔记开头的 `<style>` | 这一章特有的调整 | 某章要深色封面 |
...
**优先级**：外部 CSS 先加载、笔记内 `<style>` 后加载，所以本篇的写法总能覆盖课程主题——
想让某一章的封面换个颜色，不用去动主题文件。
