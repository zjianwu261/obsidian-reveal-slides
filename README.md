...
你不用凭空描述——**这一页的文字你早就写好了**。指着它说一句：

> 给「算术运算符」这一页配张图

模型会读这一页的 grid 正文和 `note:` 讲稿（讲稿里的结论往往更全），判断该画哪一种，
写出声明、渲染、把 ```` ```svg ```` 交给你，**并告诉你正文里哪几条现在该删掉**。

最后这件事最关键。图和文字讲同一件事，观众不知道该看哪边——这正是"割裂感"的来源。
正确的分工是：**图讲结构，文字只留图上没有的**（结论、易错点、考题）。

比如这一页原来的正文是三组九条要点，图画出来之后，正文该缩成两句：
