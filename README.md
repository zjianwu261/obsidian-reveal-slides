...
**配了不止一套对话接口时，对话框版式条的右端会出现一个下拉框**，不用跑设置页也能换。
...
预览面板下方分两半，上面一条切「配图 / 对话」。配图这半边是一条流水线：

```
┌ 这一页 ─────────┐        ┌ 配图描述（可以改）──┐
│ 题目：4.1 自增… │ 想一张 │ 右边先把 1+2 算成 3， │
│ 讲稿：我们刚才… │   →    │ 再倒进原本装着 1 的… │
└─────────────────┘        └──────────────────────┘
...
像好几个人拼的。五套都禁掉了文字、边框、水印：生成模型写不好字，中文尤其糊成一团。
...
#### 两个斜杠命令：/svg 和 /abstract

「对话」那半边，输入框里打 `/` 弹出候选，上下键选、回车填进输入框。填进去的只是
`/svg` 这几个字符，后面可以接着写你要补的话（`/svg 把比喻换成传送带`）。

| 命令 | 出什么 |
| --- | --- |
| `/svg` | 一张**矢量**示意图：讲结构、能改字、缩放不糊 |
| `/abstract` | 这一页的正文大纲（两级列表、10 行内） |

**两条都从 `note:` 讲稿出发，而且都不动讲稿。** 讲稿是你真正想讲的东西，幻灯片上那几行字
是从它压缩出来的结果——让模型改压缩结果，只会把话越缩越干。
...
#### 三个斜杠命令：/fig、/svg、/abstract

输入框里打 `/` 弹出候选，上下键选、回车填进输入框。填进去的只是 `/svg` 这几个字符，
后面可以接着写你要补的话（`/svg 把比喻换成传送带`）——那一大段规矩是给模型看的，
铺在眼前只会挡住你自己要写的东西。

| 命令 | 出什么 | 走哪个接口 |
| --- | --- | --- |
| `/fig` | 一张**位图**配图：比喻场景、有质感，图里不带字 | 画图接口 |
| `/svg` | 一张**矢量**示意图：讲结构、能改字、缩放不糊 | 对话接口 |
| `/abstract` | 这一页的正文大纲（两级列表、10 行内） | 对话接口 |

**三条都从 `note:` 讲稿出发，而且都不动讲稿。** 讲稿是你真正想讲的东西，幻灯片上那几行字
是从它压缩出来的结果——让模型改压缩结果，只会把话越缩越干；让它回到讲稿，才谈得上重新组织。

怎么选：**结构走 `/svg`，气氛走 `/fig`**。流程、对比、寄存器位分布、时序这类东西
要的是准确和可改，位图做不到；而"先取旧值再自增像柜台先递旧账单"这种画面，
矢量图画不出质感。

`/fig` 是两步走的：先让**对话模型**读 `note:` 讲稿，把"这一页在讲哪个过程或差别、
画面怎么摆才看得出来"写成一句英文提示词，再交给**画图模型**画。直接让画图模型
"按这一页配图"是不行的——它没读过讲稿，只会给你一张泛泛的科技感插画。

交给模型的东西里，**标题条和 `note:` 讲稿会被单独拎到最前面**：题目划定范围，
讲稿决定内容（混在整页源码里，它多半只扫一眼就动笔）。两样都要——讲稿常常从上一页的
话头讲起、末尾又拐到下一页去，题目是那把尺子，跟题目无关的话一句都不画。
幻灯片正文只是压缩过的结论，照着它画只能画出几个名词。提示词里还写死了
几条：要写出画面里具体有什么、谁在做什么动作（`concept of increment` 这种抽象名词
只会换来一堆发光的电路和齿轮）；最多两格对照；`no text`——生成模型写不好字，
中文尤其糊成一团，说明的字由正文承担。

画出来的图存进 `assets/<笔记名>/`，**直接写回这一页**，不再拿一段 markdown 问你要不要。
图早就存成文件了，摆出来让你确认的只是一行引用，真正要看的是预览里那张图——
不满意就再来一次，或 ⌘/Ctrl + Z 撤销。正文和讲稿一个字不动。

**画幅从你自己写的 `<grid dim>` 里拿。** 先把标题、`note:`、正文、和空的 fig 格子写好：
...
`/fig` 就按 58×66 这个比例要一张方图**填进去**，标签上的 `dim`/`pos` 一个字不动——
版是你排的，插件没有理由替你改。这一页压根没有 fig 格子时才新开一格。

**风格是定死的**，不由模型每次自己想：扁平矢量、一蓝一橙加中性灰、白底、大量留白、
没有渐变阴影 3D、没有齿轮和发光电路、没有文字。每页自己想风格等于每页抽一次卡，
一本课件会看着像好几个人拼的。改风格就改 `IMAGE_STYLE` 那一段（或你自己的提示词文件）。

**动手之前它会先说一句要画什么**，中文，一句话——一张图要跑一分钟，
跑完才发现跑偏了太亏。不对就 ⌘Z 撤销、换句补充语再来。
...
**画幅从你自己写的 `<grid dim>` 里拿。** 先把 `note:`、正文、和空的 fig 格子写好：
...
`/fig` 就按 58×66 这个比例要一张方图填进去，**标签上的 `dim`/`pos` 一个字不动**——
那是你自己排的版，插件没有理由替你改。只有这一页压根没有 fig 格子时，
才按版式条选的那一档新开一格。
...
| 版式 | 什么时候用 |
| --- | --- |
| 图上文下 | flow / bitfield / timeline 这类宽扁的图 |
| 文上图下 | 结论先摆出来，图在下面收尾 |
| 图左文右 | **默认**。图占大半、要点在右，一眼同时看见两边 |
| 文左图右 | 先读文字再看图的页面 |
| 通栏正文 | 不配图，只留一页大纲 |
| 代码带说明 | 代码在左，右边一句一句讲 |
...
打开就选着**图左文右**，换过之后记住你换的那一档（每页从头挑一次没有意义，
课件的排法是稳定的）。点中之后再打字，两句话一起发；**什么都不打直接回车，
就是照这个版式重排这一页**。再点一次取消选中，位置就交回给模型。
