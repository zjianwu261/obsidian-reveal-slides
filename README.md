...
- **嫌代码小，八成是 grid 太矮，不是字号没调**：代码块默认 `0.9em`（画布 1920 时 36px，
  比正文小一号），但这只是「放得下时」的字号 —— 放不下会自动往下缩，而且**只缩不放**。
  实测 19 行代码放进 `dim="45 60"` 的格子（864×648px），36px 要占 757px 高，于是被压到
  26px；同一块代码把格子开到 `dim="45 72"` 就能保住 36px。行数 × 1.25 行距 + 上下 padding
  大致就是需要的高度，按这个开格子，比反复调字号省事。
...
默认白底（GitHub Light 配色）。想要黑底，在 grid 上加一个 `dark`：

```markdown
<grid dim="45 72" pos="5 12" class="code dark">
```

底色、前景、语法高亮整套一起换成 GitHub Dark，不用自己去覆盖 `.hljs-*`。
只想微调某一个颜色的话，八个变量都可以在自己的 CSS 里改：

```css
.code pre {
  --code-bg: #0d1117;      /* 底色 */
  --code-fg: #e6edf3;      /* 普通文字 */
  --code-comment: #8b949e; /* 注释 */
  --code-keyword: #ff7b72; /* 关键字、类型 */
  --code-string: #a5d6ff;  /* 字符串 */
  --code-number: #79c0ff;  /* 数字、内置名 */
  --code-title: #d2a8ff;   /* 函数名、类名 */
  --code-deletion: #ffa198;
}
```

黑底用的是 `#1e1f26` 而不是纯黑：投影仪的黑场本来就发灰，纯黑配白字对比过硬，
一节课看下来晃眼。
...
| Draw Figure as SVG | — | 读讲稿画一张矢量示意图，填进 `class="fig"` 的 grid |
| Rewrite Abstract from Notes | — | 读讲稿总结成正文大纲，填进 `class="abstract"` |
...
「对话」那些活（`/svg` 那类改页面的请求）按一份系统提示词干：一页由哪几种块组成、正文该整理成什么样、
...
每一行最后是**用途**：「对话」出文字（正文、SVG 代码），「画图」出位图——两类走的不是
同一个接口（`/chat/completions` 与 `/images/generations`），填错了只会拿到一句看不懂的 400。
`/fig` 要画图那套，另外两条命令用对话那套，两套可以来自同一个中转站。

右边的圆点点一下就是「改用这一套」（只对话那几套有，画图接口不跟它抢这个位置）。
**配了不止一套对话接口时，工作台最上面那一条的右端会出现一个下拉框**，不用跑设置页也能换。
...
**默认是关的**，要用先去 设置 → Slide Preview → AI 助手 → **启用** 打开——
它要占预览面板下方四成的地方，不用 AI 的人不该被它挤掉半屏幻灯片。
开关当场生效，不用把预览关了再开；关着的时候，设置页里接口、提示词、超时那几项也一并收起来。

打开之后，预览面板下方就是它，一条流水线：

```
对话 deepseek-chat  ·  生图 gpt-image-2                     [接口 ▾]
课件第4章 · 第 2.4 页 · 基本运算符
┌ 这一页 ─────────┐        ┌ 配图描述（可以改）──┐
│ 题目：基本运算符 │ 想一张 │ 让学生看懂"先把右边  │
│ 讲稿：我们刚才… │   →    │ 算完再存进左边"…     │
└─────────────────┘        └──────────────────────┘
  画风  课堂教学 白板手绘 等距示意 剪纸拼贴 写实照片
                                   [ 按这段描述生图 ]
```

最上面一行写着**这会儿在用哪两个模型**：这条流水线要两个模型接力——对话的那个读讲稿
想构思、把描述译成英文，画图的那个才动笔。图不对先看这一行，多半是其中一个没配对。
右边的下拉框换对话那套（配了不止一套才出现）。

**左边只读**——题目从标题条来，讲稿从 `note:` 来，跟着当前页自动换。要改就回笔记里改，
那儿才是原稿。

**中间「想一张」**：让 AI 读题目和讲稿，想出一段中文的画面描述。描述框里已经写着东西时，
它会当成"按这个方向再想一遍"的交代。

**右边可以改**——这一步的价值全在这儿。模型想的构思十有八九差一点意思（挑错了重点、
比喻不贴），而一张图要跑一分钟；**跑完才发现构思不对等于白等**。先摆出来给你改，改完再画。
也可以完全不用 AI，自己直接写画面。

描述是**五行**，不是一段话——分开写是为了好改：不满意换个比喻就重写「场景」那一行，
嫌乱就删「画面」里的一件，别的不动。

```
依据：初学者经常会少写一个等号，导致程序逻辑完全出错。
场景：售票窗口前的一条队伍。
构图：从左到右三步。
...
**第一行「依据」是从讲稿里原样抄的一句**，不许概括、不许改写——它是给你一眼核对用的：
这一页讲了五件事，它挑的是哪一件，看这行就知道。挑错了当场就能看出来，
不用等一分钟画完才发现跑偏。翻译那步会跳过它（中文原话翻进去，画出来是一堆标语）。

规矩写得很死，因为生图模型有几样固定的短板：

| 规矩 | 为什么 |
| --- | --- |
| 一句一件事，三到五句 | 一句里塞满分号和"随后""才"，画图的读不懂，画出来是一团 |
| 不要靠数量表达 | 生成模型数不准个数，三个以上必错。多与少用大小、满与空、位置先后表示 |
| 不出现数字、字母、变量名 | 图里一个字都不会有。要区分两个东西就用形状和颜色 |
| 画面里的东西不超过六样 | 数一数写了几个名词，超了就删 |
| 写"看得见的东西" | 描述是给一个没上过这门课的画师看的，不是解释代码含义 |

**画风五套**，同一段描述换套画风就是另一张图：

| 画风 | 什么时候用 |
| --- | --- |
| 课堂教学 | 默认。扁平矢量、一蓝一橙，最像课件里的图 |
| 白板手绘 | 像老师现场画的，讲推演过程时最自然 |
| 等距示意 | 有层次的结构图，讲硬件和数据流向时好用 |
| 剪纸拼贴 | 有质感、不呆板，讲比喻场景时出彩 |
| 写实照片 | 实物、器件、真实场景；抽象概念别用 |

风格不写进描述里，由插件统一追加——每页自己想风格等于每页抽一次卡，一本课件会看着
像好几个人拼的。五套都禁掉了文字、边框、水印。

**「按这段描述生图」**：中间那道中译英只翻译不创作（你删掉的东西不能被它偷偷加回去），
接上画风发给画图接口，图存进 `assets/<笔记名>/`，**直接填进这一页的 fig 格子**。
画幅从你自己写的 `<grid dim>` 里拿，标签上的 `dim`/`pos` 一个字不动。

所以你的流程是：先把标题、`note:`、正文和空的 fig 格子写好——
...
不在面板上（面板留给配图那条流水线），在命令面板里：

| 命令 | 做什么 |
| --- | --- |
| Draw Figure as SVG | 读讲稿画一张**矢量**示意图，填进 `class="fig"` 的 grid |
| Rewrite Abstract from Notes | 读讲稿总结成正文大纲（两级列表、10 行内），填进 `class="abstract"` |

两条也都从 `note:` 讲稿出发、都不动讲稿，改完直接写回（不满意 ⌘/Ctrl + Z）。
它们按[库里那份提示词](#ai-提示词放在库里随时自己改)干活。

怎么选：**结构走 SVG，气氛走配图**。流程、对比、寄存器位分布、时序这类东西要的是准确
和可改，位图做不到；而"先取旧值再自增像柜台先递旧账单"这种画面，矢量图画不出质感。
...
现成的例子：[`examples/三极管分类.html`](examples/三极管分类.html)——切换 NPN/PNP 看箭头翻向、
点各极看分工、勾电流方向、还能"考考你"。零依赖单文件，拷进 assets 就能用。
...
预览时同一时刻只有一两页在布局里，图大一点感觉不出来；打印视图不一样 ——
**整份课件的所有页会同时展开**，每一张图都要在这一刻解码成位图。

拿一章真实课件量过：26 张插图，文件合计 72 MB，尺寸多在 1800~2500px，
解码成位图约 **373 MB**，全部压在一次布局里。慢的是这个，不是幻灯片页数。

这些图大多是截图直接粘进笔记的，PNG 没优化过（约 2 字节/像素），
而它们在 1920 画布上顶多显示到 800px 宽。用 `pngquant` / `oxipng` 之类压一遍，
或者贴图前先缩到显示尺寸的两倍，通常能小 5~10 倍 —— 导出、预览、同步一起变快。
