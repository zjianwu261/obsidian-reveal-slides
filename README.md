...
`/fig` 是两步走的：先让**对话模型**读讲稿，把"这一页要让人看懂哪件事、用什么比喻"
写成一句英文提示词，再交给**画图模型**画。直接让画图模型"按这一页配图"是不行的——
它没读过讲稿，只会给你一张泛泛的科技感插画。提示词里写死了 `no text`：
生成模型写不好字，中文尤其糊成一团，说明的字由正文承担。

画出来的图存进 `assets/<笔记名>/`，页面里只多一行引用，**正文和讲稿一个字不动**。
